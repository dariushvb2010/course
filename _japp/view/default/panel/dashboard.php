...
<td><?php tr("Number of Users");?> : <?php $x=j::SQL("SELECT COUNT(*) AS Count FROM ".reg("jf/users/table/name")); echo $x[0]['Count'];?></td>
...
<?php
$req=j::SQL("SELECT S.*,U.* FROM `".reg("jf/session/table/name")."` AS S LEFT JOIN `".reg("jf/users/table/name")."` AS U ON (
        	S.`".reg("jf/session/table/UserID")."`=U.`".reg("jf/users/table/UserID")."`) ORDER BY S.".reg("jf/session/table/AccessCount")." DESC");
